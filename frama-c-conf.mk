...
n: total
	@echo $(LIBNAME)
	#cp $(TARGET).total/framac.c $(TARGET).total/framac_bak.c
	cat $(TARGET).total/framac_bak.c $(CONTIKI)/prepare_memory.c $(CONTIKI)/jnimain.c > $(TARGET).total/framac.c
	sed -i -e '665,700{s/^/\/\//g}' $(TARGET).total/framac.c
	sed -i -e 's/typedef char int8_t;/typedef signed char int8_t;/g' $(TARGET).total/framac.c
